...
def find_phase_space(length_1, mass_1, length_2, mass_2, steps, time, phasespace_step_size=10, file_name=None):
    ...
    phasespace_step_size : int, optional
        Number of points to calculate for each angle. Default is 10 per angle totalling up to 100 points.
...
def make_plot(pendulum_data, plot_energy=False, save=False):
    '''
    Uses the final data generated by the "make_data" function to generate plots.

    Parameters
    ----------
    pendulum_data : Pandas Dataframe, required
        A pandas dataframe from which the data to plot is extracted.
    plot_energy: Boolean, optional
        Makes an energy plot if True, else does not.
    save: Boolean, optional
        Saves the plots generated if True, else does not.
...
def save_data(pendulum_data, file_name):
    '''
    Saves the pandas dataframe as a file.
...
def fetch_data(file_name):
    '''
    Reads in a pandas dataframe from the specified location
